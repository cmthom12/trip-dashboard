 ...
 *   4. updates all FOUR traveler-name lists (family is in the JSON itself;
 *      ALLOWED + PLANNERS in server.js; PLANNERS in public/index.html),
 *   5. optionally sets DEFAULT_TZ,
...
const jsonText = JSON.stringify(trip, null, 1);
...
html = patchList(html, 'PLANNERS', 'public/index.html');

// step 5 (prepared): timezone
if (tz != null) {
  const re = /const\s+DEFAULT_TZ\s*=\s*"[^"]*"/;
  if (!re.test(html)) fail('Could not find DEFAULT_TZ in public/index.html. Nothing was changed.');
  html = html.replace(re, 'const DEFAULT_TZ = ' + JSON.stringify(tz));
}
...
if (tz != null) console.log('✓ Timezone set to ' + tz);
